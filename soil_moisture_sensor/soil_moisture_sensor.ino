...
int buzzer = 8;
const int soilMoisturePin = A0;  // Analog input from soil sensor
...
void loop() {
  ...
  if (moistureValue > moistureThreshold) {
    playMelodyWhileDry();  // Plays song while moisture is low
  } else {
    noTone(buzzer);  // Make sure buzzer is off
  }

  delay(200);  // Slight delay to reduce analog noise
}

void playMelodyWhileDry() {
  for (int thisNote = 0; thisNote < notes * 2; thisNote += 2) {
    int moistureValue = analogRead(soilMoisturePin);
    if (moistureValue <= moistureThreshold) {
      noTone(buzzer); // Stop playing immediately
      return;
    }
